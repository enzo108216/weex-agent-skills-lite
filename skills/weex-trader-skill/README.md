...
- submit only after a subsequent, independent user message exactly matches `user_confirmation.reply_text` from the latest preview. Trading-mode selection or missing-field completion is not confirmation, and any order-field or trading-mode change invalidates the previous preview
- If the safe preview path is unavailable, fail closed; never fall back to the direct contract or spot order CLI. Direct CLI examples remain available only for explicitly invoked local scripts outside the natural-language Agent order flow

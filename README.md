...
- Private balances, available/frozen amounts, positions, orders, and fills with explicit `真实盘` or `模拟盘` separation.
...
Every conversational order uses a guard preview and the exact confirmation text from the latest preview. Real trading requires `--confirm-live`; official Futures demo writes require `--trading-mode demo --confirm-demo`. Credentials, internal account IDs, and raw signed headers are never returned.

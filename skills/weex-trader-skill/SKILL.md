...
- Product-rule and risk validation, structured preview, exact independent confirmation, order placement/cancellation, official conditional orders, TP/SL, order status, positions, and trade details.
...
The following are intentionally not conversational capabilities of this project: replay collection for analysis, trading profiles, deep account-risk interpretation, PnL monitor tasks, local automatic-close loops, Partner/referral queries, or Partner writes. A price-threshold close must use an official WEEX conditional order through Trader; do not create a local monitor.
...
- `scripts/weex_trade_guard.py`: `preview-order`, `preview-tp-sl`, `confirm-order`, `confirm-tp-sl`, and internal account-risk checks for safe order flows.
...
2. Call `preview-order` or `preview-tp-sl`; never call direct `place-order` from the conversational path.
3. Return the guard's `user_confirmation.reply_instruction` verbatim, including environment prefix, risk status, order summary, warnings, exact reply text, and any mode-switch text.
4. Submit only after a subsequent independent user message exactly matches the latest `user_confirmation.reply_text`. Keep `intent_id` and `risk_signature` internal. Any changed field, mode, expired intent, mismatch, stale data, or unavailable safe path rejects submission.
...
- Every request includes `trade_types` (`SPOT`, `FUTURES`, or both), symbols or all symbols, maximum conservative U amount per leg, cumulative conservative U quota, and explicit `valid_hours` (1–720). Do not derive the cumulative quota from frequency, balance, validity, target amount, or expected order count.
...
- Only explicit official Spot/Futures order operations enter the automatic path. Fresh product, market, depth, fee, leverage, conversion, balance, scope, quota, and risk facts are required. Missing, stale, degraded, incomplete, unconvertible, or unproven facts return to manual preview and confirmation.

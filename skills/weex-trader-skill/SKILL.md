...
- Submission is allowed only after a subsequent, independent user message exactly matches `user_confirmation.reply_text` from the latest preview. Trading-mode selection or missing-field completion is not confirmation, and changing any order field or trading mode invalidates the previous preview.
- If the safe preview path is unavailable, fail closed; never fall back to the direct contract or spot order CLI. Direct CLI examples are for explicitly invoked local scripts, not the natural-language Agent order flow.

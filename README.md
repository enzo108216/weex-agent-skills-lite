...
Use the saved profile/Application Vault or inject the complete `WEEX_API_KEY`, `WEEX_API_SECRET`, and `WEEX_API_PASSPHRASE` set through the OpenClaw runtime. Never put secrets on argv or in chat. Every conversational order uses `preview-order` first and requires the exact confirmation text from the latest preview; real trading requires `--confirm-live`, and official futures demo writes require `--trading-mode demo --confirm-demo`.
